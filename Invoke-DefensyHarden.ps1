Function Invoke-DefensyHarden {

    <#
    .SYNOPSIS

        Invoke-DefensyHarden - Checks and hardens your Windows configuration
    ...
    .DESCRIPTION
        ...
        DefensyHarden supports hardening of a Windows system. The configuration of the system is
        retrieved and assessed using a finding list. In addition, the system can be hardened according
        to predefined values. DefensyHarden reads settings from the registry and uses other modules
        to read configurations outside the registry. 


    .PARAMETER FileFindingList

        Path to a finding list in CSV format. DefensyHarden has one list each for machine and user settings.


    .PARAMETER Mode
        
        The mode Config only retrieves the settings, while the mode Audit performs an assessment of the settings.
        The mode HailMary hardens the system according to recommendations of the DefensyHarden list.
    ...
    .PARAMETER Log
        ...
        name is assigned by DefensyHarden itself and the file is stored in the same directory as the script.
    ...
    .PARAMETER Report
        ...
        The file name is assigned by DefensyHarden itself and the file is stored in the same directory as the script.
    ...
    .EXAMPLE
        ...
        Description: DefensyHarden performs an audit, saves the results and creates a log file:
        Invoke-DefensyHarden -Mode Audit -Log -Report

        Description: DefensyHarden performs an audit with a specific list and does not show machine information:
        Invoke-DefensyHarden -FileFindingList .\lists\finding_list_0x6d69636b_user.csv -SkipMachineInformation

        Description: DefensyHarden ready only the setting with the default list, and saves the results in a specific file:
        Invoke-DefensyHarden -Mode Config -Report -Report C:\tmp\my_DefensyHarden_report.log
    ...
    # Start Main
    #
    $DefensyHardenVersion = "0.6.0-1621229896"
    ...
    If ($Log.IsPresent -and $LogFile.Length -eq 0) {
        $LogFile = "DefensyHarden_log_"+$Hostname+"_"+$ListName+"-$FileDate.log"
    }
    If ($Report.IsPresent -and $ReportFile.Length -eq 0) {
        $ReportFile = "DefensyHarden_report_"+$Hostname+"_"+$ListName+"-$FileDate.csv"
    }
    If ($Report.IsPresent) {
    ...
    Write-Output "`n"
    Write-Output "      =^._.^="
    Write-Output "     _(      )/  DefensyHarden $DefensyHardenVersion"
    Write-Output "`n"    
    Write-ProtocolEntry -Text "Starting DefensyHarden" -LogLevel "Info"
    ...
    If ($Mode -eq "Audit" -or $Mode -eq "Config") {
        ...
        # A CSV finding list is imported. DefensyHarden has one machine and one user list.
        ...
        ForEach ($Finding in $FindingList) {
            ...
            # to a file, which means DefensyHarden must create a temporary file
            # and afterwards delete it. DefensyHarden is very orderly.            
            #
    ...
    #
    # Start HailMary mode
    # DefensyHarden configures all settings in a finding list file.
    # Even though DefensyHarden works very carefully, please only
    ...
    Write-Output "`n"
    Write-ProtocolEntry -Text "DefensyHarden is done" -LogLevel "Info"
    ...
    If ($Mode -eq "Audit") {
        ...
        If ($ScoreTotal -ne 0 ) {
            $DefensyHardenScore = ([int] $ScoreAchived / [int] $ScoreTotal) * 5 + 1
        }        
        $DefensyHardenScoreRounded = [math]::round($DefensyHardenScore,2)

        # Overwrite DefensyHarden Score if no finding is passed
        If ($StatsPassed -eq 0 ) {
            $DefensyHardenScoreRounded = 1.00
        }

        If ($StatsError -gt 0) {
            Write-ProtocolEntry -Text "During the execution of DefensyHarden errors occurred due to missing admin rights or tools. For a complete result, these errors should be resolved. Total errors: $StatsError" -LogLevel "Error"
        }
            
        Write-ProtocolEntry -Text "Your DefensyHarden score is: $DefensyHardenScoreRounded. DefensyHarden Statistics: Total checks: $StatsTotal - Passed: $StatsPassed, Low: $StatsLow, Medium: $StatsMedium, High: $StatsHigh." -LogLevel "Info"
    }
    Write-Output "`n"
}